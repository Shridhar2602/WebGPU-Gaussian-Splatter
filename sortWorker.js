...
function sortGaussiansByDepth(gaussians, viewMatrix, flag) {

	let t = performance.now() / 1000;
	console.log(flag)
	const calcDepth = (i) => 
		flag*(gaussians.positions[i * 3] * viewMatrix[8] +
	...
	function getDepthFromViewMatrix(i) {
		...
		const transposedViewMatrix = mat4.create();
		mat4.transpose(transposedViewMatrix, viewMatrix);
		const depthAxis = mat4.getAxis(mat4.transpose(transposedViewMatrix), 2)
		const depth = vec3.dot(depthAxis, point);
	...
	function mergeSortIndices() {
		let indices = Array.from({ length: gaussians.count }, (_, i) => i);
		return mergeSort(indices, (a, b) => getDepthFromViewMatrix(a) - getDepthFromViewMatrix(b));
	}
...
self.onmessage = function(event) {
    ...
    const sortedData = sortGaussiansByDepth(data.gaussians, data.viewMatrix, data.flag); // Example sorting logic
